...
for line in lines:
    ...
    if match:
        ...
        iters.append(start)
        if G_rec_loss > 0.5:
            G_rec_losses.append(0.5)
        else:
            G_rec_losses.append(G_rec_loss)
        if G_lp_loss > 0.5:
            G_lp_losses.append(0.5)
        else:
            G_lp_losses.append(G_lp_loss)
        if G_GAN_loss > 1:
            G_GAN_losses.append(0.5)
        else:
...
plt.ylabel('Loss')
plt.title('Training Loss for M3_FFHQ_pretrained_BC.jpg')
plt.legend()
plt.savefig('M3_FFHQ_pretrained_BC.jpg')