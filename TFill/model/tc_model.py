...
class TC(BaseModel):
    ...
    def set_input(self, input):
        ...
        # plt.imshow(self.img_org[0].permute(1, 2, 0).cpu().numpy())
        # plt.show()
        self.img = input['img'].to(self.device) * 2 - 1
        # plt.imshow(self.img[0].permute(1, 2, 0).cpu().numpy())
        # plt.show()
        self.mask = input['mask'].to(self.device)
        # plt.imshow(self.mask[0].permute(1, 2, 0).cpu().numpy())
        # plt.show()

        # get I_m and I_c for image with mask and complement regions for training
        self.img_m = self.mask * self.img_org
        # plt.imshow(self.img_m[0].permute(1, 2, 0).cpu().numpy())
        # plt.show()
