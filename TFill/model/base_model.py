...
class BaseModel():
    ...
    def setup(self, opt):
        ...
        if not self.isTrain or opt.continue_train:
            ...
            # modified the print_networks to save teh architecture to a separate file
            self.print_networks()
    ...
    def print_networks(self):
        ...
        # write the architecture to the architecture.txt file
        with open('E:/1-EngSci3/1-ECE324/Triple-Dots/TFill/model/architecture.txt', 'w') as file:

            file.write('---------- Networks initialized -------------')
            for name in self.model_names:
                if isinstance(name, str):
                    net = getattr(self, 'net' + name)
                    num_params = 0
                    ...
                    file.write(str(net))
                    file.write('[Network %s] Total number of parameters : %.3f M' % (name, num_params / 1e6))
            file.write('-----------------------------------------------')
